...
func InitPodWebhook(
	...
	if err = addPodMutatingWebhook(admissionClient, mutatingWebhookName); err != nil {
		return err
	}
...
func (pw *podWebhook) Default(ctx context.Context, obj runtime.Object) error {
	...
	_, ok := pod.Annotations[constant.AnnoMutatingPodWebhook]
	if !ok {
		return nil
	}

	mutateLogger.Sugar().Debugf("Pod %s/%s is annotated with %s, start injecting network resources", pod.Namespace, pod.GenerateName, constant.AnnoMutatingPodWebhook)
	podNetworkMutatingWebhook(pw.spiderClient, pod)
